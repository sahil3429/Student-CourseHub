...
try {
    if ($type === 'admin') {
        ...
        $stmt = $conn->prepare("SELECT su.id, su.Username, s.Name 
                              FROM staff_users su
                              JOIN Staff s ON su.id = s.StaffID
                              WHERE su.id = :id");
        $stmt->bindParam(':id', $id);
        $stmt->execute();
        $user_info = $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    if (empty($new_password) || empty($confirm_password)) {
        ...
        try {
            if ($type === 'admin') {
                ...
                $stmt = $conn->prepare("UPDATE staff_users SET Password = :password WHERE id = :id");
                $stmt->bindParam(':password', $hashed_password);
                $stmt->bindParam(':id', $id);
                $stmt->execute();
            }
